...
<svelte:head><title>Daily Quiz — Labbook</title></svelte:head>

<PageHeader title="Daily Quiz" subtitle="A little practice. A deeper understanding.">
	{#snippet actions()}
		{#if dash?.latest_quiz_id}
			<Button href={`/quiz/${dash.latest_quiz_id}`} variant="secondary" size="md"
				>Latest quiz</Button
			>
		{/if}
	...
	<div class="mt-8">
		...
		<section>
			<div class="mb-3 flex items-center justify-between">
				...
				<a href={resolve('/history')} class="text-sm font-medium text-primary hover:underline"
					>View all</a
				>
			</div>
			{#if recent.length === 0}
